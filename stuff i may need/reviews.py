    ...
    if data:                
        for r in data['reviews']:
            ...
            try:
                ...
                pass
    urlnew = "https://www.amazon.in" + data["next_page"]
    data = scrape(urlnew)

# product_data = []
with open("urls.txt",'r') as urllist, open('data.csv','w', encoding="utf-8", errors="ignore", newline="") as outfile:
    writer = csv.DictWriter(outfile, fieldnames=["title","content","date", "author","rating","product","url"],quoting=csv.QUOTE_ALL)
    writer.writeheader()
